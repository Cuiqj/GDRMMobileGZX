...
-(void)viewDidLoad{
    ...
    if (![self.caseID isEmpty]) {
        self.caseCount = [CaseCount caseCountForCaseInfoId:self.caseID];
        
        [self pageLoadInfo];
    }
    [self.textHappenTime addTarget:self action:@selector(selectTime:) forControlEvents:UIControlEventTouchDown];
    [super viewDidLoad];
}
...
- (void)pageLoadInfo{
    ...
    if(self.caseCount)
        self.textHappenTime.text = [dateFormatter stringFromDate:self.caseCount.happenTime];
    else
        self.textHappenTime.text = [dateFormatter stringFromDate:caseInfo.happen_date];
    if (citizen) {
...
- (void)pageSaveInfo{
    ...
    [dateFormatter setDateFormat:@"yyyy年MM月dd日HH时mm分"];
    [dateFormatter setLocale:[NSLocale currentLocale]];
    self.caseCount.happenTime=[dateFormatter dateFromString:self.textHappenTime.text];
    [[AppDelegate App] saveContext];
}

-(void) selectTime:(UITextField *) sender{
    DateSelectController *dateselecterVC=[self.storyboard instantiateViewControllerWithIdentifier:@"datePicker"];
    if([self.pickVc isAccessibilityElement]){
        [self.pickVc dismissPopoverAnimated:YES];
        self.pickVc = nil;
    }
    dateselecterVC.pastDate=[NSDate date];
    dateselecterVC.delegate=self;
    dateselecterVC.pickerType=1;
    self.pickVc= [[UIPopoverController alloc] initWithContentViewController:dateselecterVC];
    [self.pickVc presentPopoverFromRect:sender.frame inView:self.view permittedArrowDirections:UIPopoverArrowDirectionLeft animated:YES];
    dateselecterVC.dateselectPopover=self.pickVc;
 }
-(void)setPastDate:(NSDate *)date withTag:(int)tag{
    self.caseCount.happenTime=date;
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy年MM月dd日HH时mm分"];
    [dateFormatter setLocale:[NSLocale currentLocale]];
    self.textHappenTime.text=[dateFormatter stringFromDate:date];
}
//根据记录，完整默认值信息
