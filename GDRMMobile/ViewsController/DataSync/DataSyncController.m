...
- (IBAction)btnUpdateDocFormat:(UIButton *)sender {
    ...
    if([orgName rangeOfString:@"大队"].location !=NSNotFound){
        ...
        zhongdui=@"3";
    }//NSURL *url=[NSURL URLWithString:[NSString stringWithFormat:@"%@app/XML/%@/%@.xml",[AppDelegate App].serverAddress ,zhongdui,docXMlName]];
    for(NSString *docXMlName in docArray){
        NSString * remotePath=[NSString stringWithFormat:@"%@app/XML/%@/%@%@", [AppDelegate App].serverAddress,zhongdui,docXMlName,@".xml"];
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
        NSString *libraryDirectory = [paths objectAtIndex:0];
        ...
        NSError *error;
        //        [[NSFileManager defaultManager] removeItemAtPath:destPath error:&error];
        //        [[NSFileManager defaultManager] copyItemAtURL:[NSURL URLWithString:remotePath] toURL:[NSURL URLWithString:destPath] error:&error];
        NSURL *url=[NSURL URLWithString:[NSString stringWithFormat:@"%@app/XML/%@/%@.xml",[AppDelegate App].serverAddress ,zhongdui,docXMlName]];
