    ...
    switch (self.pickerType) {
        ...
        case 3:{
            ...
            if(self.dataArray && [self.dataArray count] > 0){
                for (InquireAnswerSentence *inquireAnswerSentence in self.dataArray) {
                    if ([inquireAnswerSentence.sentence rangeOfString:@"XXXX年XX月XX日XX时XX分"].location != NSNotFound) {
                        if ([inquireAnswerSentence respondsToSelector:@selector(setSentence:)]) {
                            NSLog([NSString stringWithFormat:@"%@%@",@"答：明白，",[self.delegate caseDescription]] );
                            [inquireAnswerSentence setValue:[NSString stringWithFormat:@"%@%@",@"答：明白，",[self.delegate caseDescription]] forKey:@"sentence"];
                        }
                    }
                    if([askID isEqualToString:@"SYS201600042"]){
                        [inquireAnswerSentence setValue:[NSString stringWithFormat:@"%@",[self.delegate GeRenOrGongSi]] forKey:@"sentence"];
                    }
                }
            }
        }
        ...
        case 5:{
            ...
            NSFetchRequest *fetchRequest=[[NSFetchRequest alloc] init];
            //[fetchRequest setEntity:entity];
            NSPredicate *predicate=[NSPredicate predicateWithFormat:@"code_name=='询问笔录模板'"];
            [fetchRequest setEntity:entity];
            [fetchRequest setPredicate:predicate];
            //NSSortDescriptor *sortDescriptor=[NSSortDescriptor sortDescriptorWithKey:@"the_index.integerValue" ascending:YES];
            self.dataArray=[context executeFetchRequest:fetchRequest error:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (self.pickerType) {
        ...
        case 5:{
            ...
            cell.textLabel.numberOfLines=0;
            cell.textLabel.font=[UIFont systemFontOfSize:17];
            cell.textLabel.text=[[self.dataArray objectAtIndex:indexPath.row] valueForKey:@"type_value"];
        }
            break;
        default:
            break;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (self.pickerType) {
        ...
        case 3:
            ...
            break;
        case 5:
            //[self.delegate setAskSentence:cell.textLabel.text withAskID:[[self.dataArray objectAtIndex:indexPath.row] valueForKey:@"type_value"]];
            [self.delegate setMuban:cell.textLabel.text withMubanType_Value:[[self.dataArray objectAtIndex:indexPath.row] valueForKey:@"type_value"] ];
            break;
        default:
            break;
    }
