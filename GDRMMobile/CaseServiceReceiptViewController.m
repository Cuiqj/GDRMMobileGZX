...
- (void)pageLoadInfo{
    ...
    self.textreason.text = proveInfo.case_long_desc;//[NSString stringWithFormat:@"%@车%@", citizen.automobile_number, proveInfo.case_short_desc];
...
- (void)generateDefaultInfo:(CaseServiceReceipt *)caseServiceReceipt{
    ...
    caseServiceReceipt.service_position =[NSString stringWithFormat:@"%@%@%@",@"广珠西线高速公路",caseInfo.side,stationString];// caseInfo.full_happen_place;
...
- (NSURL *)toFullPDFWithPath:(NSString *)filePath{
    ...
    if (![filePath isEmpty]) {
        ...
        self.caseServiceReceipt.service_company=[[self.caseServiceReceipt.service_company
          stringByReplacingOccurrencesOfString: @"一中队" withString: @"" ] stringByReplacingOccurrencesOfString: @"二中队" withString: @"" ];
