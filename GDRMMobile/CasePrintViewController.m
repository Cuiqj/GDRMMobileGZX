...
-(NSString *)xmlStringFromFile:(NSString *)xmlName{
    ...
    if([userDefaults boolForKey:@"isDebugingDocFormat"]){
        xmlString=[self xmlStringFromUrlFile:xmlName];
    }else{
        xmlString=[self xmlStringFromLocalFile:xmlName];
    }
    return xmlString;
}

-(NSString *)xmlStringFromUrlFile :(NSString *)xmlName{
    NSString *orgName=[[OrgInfo orgInfoForSelected] valueForKey:@"orgshortname"];
    NSString *zhongdui=@"";
    if([orgName rangeOfString:@"大队"].location !=NSNotFound){
        zhongdui=@"0";
    }else if([orgName rangeOfString:@"一中队"].location !=NSNotFound){
        zhongdui=@"1";
    }else if([orgName rangeOfString:@"二中队"].location !=NSNotFound){
        zhongdui=@"2";
    }else if([orgName rangeOfString:@"三中队"].location !=NSNotFound){
        zhongdui=@"3";
    }
    NSURL *url=[NSURL URLWithString:[NSString stringWithFormat:@"%@app/XML/%@/%@.xml",[AppDelegate App].serverAddress ,zhongdui,xmlName]];
    //NSURL *url=[NSURL URLWithString:[NSString stringWithFormat:@"%@app/XML/%@.xml",[AppDelegate App].serverAddress ,xmlName]];
...
-(NSString *)xmlStringFromLocalFile :(NSString *)xmlName{
    NSError *error=nil;
        ...
        if (dataTable) {
            ...
            while (uitextfield) {
                ...
                if (contentInXML) {
                    ...
                    //遍历完context 得到实际的字符串contentString
