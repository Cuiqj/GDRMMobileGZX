...
@synthesize textDegree = _textDegree;
...
//        datePicker.code_name=@"路产损坏描述";
    ...
    [self pickerPresentForIndex:22 fromRect:[(UITextField*)sender frame]];
}
-(void)setCaseText:(NSString *)aText{
    self.textDegree.text=aText;
}
- (void)setSelectData:(NSString *)data{

    self.textDegree.text=data;
}
//数量输入变化时自动计算金额
-(void)textFieldTextDidChange:(NSNotification *)aNotifocation{
    self.textLength.text=@"";
...
//    [self.textQuantity resignFirstResponder];
    ...
    if (self.textQuantity.text.doubleValue>0) {
        ...
        if (![self isUsingCustomRoadAsset]) {
            ...
            degree= self.textDegree.text;
        if ((![self.textLength.text isEmpty]) && (![self.textWidth.text isEmpty])) {
            remark=[NSString stringWithFormat:@"长%@米、宽%@米",self.textLength.text,self.textWidth.text];
        
        }
        [self.deformInfoVC addDeformationForRoadAsset:newRoadAssetPrice andQuantity:quantity withRemark:remark withDegree:degree];
    }
}
