...
- (NSString *)locality{
   return  [[NSString alloc] initWithFormat:@"%@高速%@%02ldKm+%03dm",[[AppDelegate App].projectDictionary objectForKey:@"cityname"] ,self.side,self.station_start.integerValue/1000,self.station_start.integerValue%1000];
}
...
- (NSString *)full_case_mark3{
    ...
    NSString *mark3String=[numFormatter stringFromNumber:[NSNumber numberWithInteger: self.case_mark3.integerValue]];
    
    return [NSString stringWithFormat:@"%@%@", lochus_code, mark3String];
}
