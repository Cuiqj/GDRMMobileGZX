...
- (IBAction)btnSwitch:(UIButton *)sender {
    if (self.descState == kAddNewRecord ||self.descState == kUnderBridgeCheck) {
...
- (IBAction)btntoUnderBridgeCheck:(UIButton *)sender {
	if (self.descState == kAddNewRecord|| self.descState == kNormalDesc) {
		[UIView animateWithDuration:0.3
							  delay:0.0
							options:UIViewAnimationOptionCurveEaseInOut
						 animations:^{
...
//        }
        ...
        if (!isBlank) {
            ...
            inspectionRecord.remark=self.textBridgeInspectDesc.text;
            [[AppDelegate App] saveContext];
            self.inspectRecordID= inspectionRecord.myid;
